...
const getAllTickets = async (req, res) => {
    ...
    try{
        let allTickets = await ticket.find({}).populate('createdBy', '-password -_id').populate('assignedTo', '-password -_id');
...
const getAllComments = async (req, res) => {
    ...
    try{
        let allComments = await comment.find({}, {_id: 0}).populate('author', '-password -_id').populate('TicketRef', '-_id -createdBy -assignedTo');
...
const createTicket = async (req, res) => {
    ...
    try{
        ...
        await ticket.updateOne({_id: ticketRef}, {$addToSet: {comments: commentId}});
...
const displayTicket = async (req, res) => {
    ...
    try{
        let returnTicket = await ticket.findById(id, {_id: 0}).populate('createdBy', '-password -_id -createdAt -updatedAt').populate('assignedTo', '-password -_id -createdAt -updatedAt');
...
const updateTicket = async (req, res) => {
    ...
    try{
        await ticket.updateOne({_id: id}, {$set: {title: ticketToUpdate.title, description: ticketToUpdate.description, 
            createdOn: ticketToUpdate.createdOn, createdBy: ticketToUpdate.createdBy, assignedTo: ticketToUpdate.assignedTo}});
...
const displayComment = async (req, res) => {
    ...
    try{
        let commentToDisplay = await comment.find({TicketRef: id}, {_id: 0}).populate('TicketRef', '-_id -createdBy -assignedTo');;
...
const updateComment = async (req, res) => {
    ...
    try{
    await comment.updateOne({_id: id}, {$set: {description: commentToUpdate.description, isInternal: commentToUpdate.isInternal, userStories: commentToUpdate.userStories}});
